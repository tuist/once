...
export async function setupPassportGraphs(root = document) {
  for (const element of root.querySelectorAll("[data-passport-graph]")) {
    ...
    svg.innerHTML = `<defs><marker id="passport-arrow" viewBox="0 0 8 8" refX="7" refY="4" markerWidth="6" markerHeight="6" orient="auto"><path d="M 0 0 L 8 4 L 0 8 z" /></marker></defs>${edges.map(edge => `<path class="passport-graph-edge" marker-end="url(#passport-arrow)" d="${(edge.sections || []).map(section => `M ${section.startPoint.x + 12} ${section.startPoint.y + 12} ${(section.bendPoints || []).map(point => `L ${point.x + 12} ${point.y + 12}`).join(" ")} L ${section.endPoint.x + 12} ${section.endPoint.y + 12}`).join(" ")}" />`).join("")}${children.map(node => `<g class="passport-graph-node" tabindex="0" data-node="${node.id}" transform="translate(${node.x + 12} ${node.y + 12})"><rect width="${node.width}" height="${node.height}" rx="8"/><text class="passport-graph-node-title" x="14" y="27">${escapeHtml(node.title)}</text><text class="passport-graph-node-detail" x="14" y="50">${node.cache === "hit" ? "Cache hit" : "Executed"} · ${node.duration_ms / 1000}s</text></g>`).join("")}`
