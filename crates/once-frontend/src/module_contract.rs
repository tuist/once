...
pub fn module_authoring_contract() -> ModuleAuthoringContract {
    ...
    ModuleAuthoringContract {
        ...
        action_primitives: vec![
            ...
            entry(
                "run_action(argv, inputs = [], outputs = [], clean_paths = [], create_dirs = [], cwd = None, env = {}, toolchain_identity = None, identifier = None, cacheable = True, inherit_parent_env = False, depends_on_prior_actions = True, stdout = None, stderr = None, sandbox = None)",
                "Declare a direct executable invocation with explicit inputs, outputs, setup, caching, and sandbox policy. `inherit_parent_env` is available only to uncached local run actions. Explicit `env` values take precedence. Use `once query validate-actions` to investigate filesystem contract drift without changing the sandbox policy.",
            ),
        ],
        test_contract: vec![
            entry(
