...
#[cfg(unix)]
#[test]
fn prelude_rust_bin_compile_omits_target_when_target_equals_host() {
    let prelude = all_prelude_source();
    let source = format!(
        r#"{prelude}
_rustc, _identity, host_triple = _rustc_toolchain("")
...
_rust_compile(ctx, "bin", "src/main.rs", "cli")
result = repr("ok")
"#
    );
    let workspace = TempDir::new().unwrap();
    let store = store_for(workspace.path(), "crates/cli");
...
#[cfg(unix)]
#[test]
fn prelude_rust_bin_compile_keeps_target_when_target_differs_from_host() {
    let prelude = all_prelude_source();
    let source = format!(
        r#"{prelude}
_rustc, _identity, host_triple = _rustc_toolchain("")
