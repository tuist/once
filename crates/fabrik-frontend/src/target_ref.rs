...
pub fn normalize_build_dep(package: &str, raw: &str) -> Result<String, TargetIdError> {
    validate_raw(raw)?;
    let base = package_components(package);
...
fn package_components(package: &str) -> Vec<String> {
    package
        .split('/')
...
mod tests {
    ...
    #[test]
    fn build_deps_resolve_from_declaring_package() {
        assert_eq!(
            normalize_build_dep("examples/macos-cli", "Greeter").unwrap(),
            "examples/macos-cli/Greeter"
        );
        assert_eq!(
            normalize_build_dep("examples/macos-cli", "./Greeter").unwrap(),
            "examples/macos-cli/Greeter"
        ...
        assert_eq!(
            normalize_build_dep("examples/macos-cli", "../shared/Logging").unwrap(),
    ...
    #[test]
    fn old_label_syntax_is_rejected_with_a_suggestion() {
        assert_eq!(
            normalize_build_dep("pkg", "//shared:Logging").unwrap_err(),
            TargetIdError::BazelSyntax {
                raw: "//shared:Logging".to_string(),
                suggestion: "shared/Logging".to_string()
            }
        );
        assert_eq!(
            normalize_build_dep("pkg", ":local").unwrap_err(),
            TargetIdError::BazelSyntax {
                raw: ":local".to_string(),
                suggestion: "local".to_string()
            }
        );
    }
