...
fn parse_shebang(line: &str, display_name: &str, path: &Path) -> Result<(String, Vec<String>)> {
    let Some(raw) = line.strip_prefix("#!") else {
        return Err(Error::Eval {
            path: display_name.to_string(),
            message: format!(
                "script {} must start with a shebang so Fabrik knows how to run it",
                path.display()
            ),
        });
    };
    ...
    if first.ends_with("/env") || first == "env" {
        ...
        return Ok(parse_fabrik_exec_shebang(runtime, runtime_args));
    }

    Ok((first.to_string(), parts.map(ToString::to_string).collect()))
}

fn parse_fabrik_exec_shebang(runtime: String, runtime_args: Vec<String>) -> (String, Vec<String>) {
    if (runtime == "fabrik" || runtime.ends_with("/fabrik"))
...
fn parse_quoted(raw: &str, name: &str, display_name: &str) -> Result<String> {
    ...
    let Some(rest) = raw.strip_prefix('"') else {
        return Err(Error::Eval {
            path: display_name.to_string(),
            message: format!("FABRIK {name} expects a quoted string"),
        });
    };
    let Some(end) = rest.find('"') else {
        return Err(Error::Eval {
            path: display_name.to_string(),
            message: format!("FABRIK {name} is missing a closing quote"),
        });
    };
    if !rest[end + 1..].trim().is_empty() {
        return Err(Error::Eval {
            path: display_name.to_string(),
            message: format!("FABRIK {name} only accepts one quoted string"),
        });
    }
...
"#,
    ...
    #[test]
    fn parses_python_ruby_and_elixir_script_annotations() {
        let tmp = TempDir::new().unwrap();
        for (name, runtime) in [
            ("build.py", "python3"),
            ("build.rb", "ruby"),
...
"#
    ...
    #[test]
    fn parses_fabrik_exec_script_shebang() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("build.py");
        fs::write(
            &path,
            r#"#!/usr/bin/env -S fabrik exec --script python3
# FABRIK input "src/**/*.py"
print("hi")
"#,
        )
    ...
    #[test]
    fn parses_fabrik_exec_separator_shebang() {
        let tmp = TempDir::new().unwrap();
        let path = tmp.path().join("build.py");
        fs::write(
            &path,
            r#"#!/usr/bin/env -S fabrik exec -- python3
# FABRIK input "src/**/*.py"
print("hi")
"#,
        )
...
"#,
        ...
        assert!(err.to_string().contains("unknown FABRIK directive"));
    }
}
