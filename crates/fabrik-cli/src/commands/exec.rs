//! `fabrik exec` - cache and execute a literal command or script file.
//!
...
//! like `<runtime> <script> [args...]` and the file carries `FABRIK`
//! headers, Fabrik treats argv as a script execution request. Most
//! users still want `fabrik run` against a declared target instead.
//!
...
fn has_fabrik_annotations(annotations: &ScriptAnnotations) -> bool {
...
fn script_input_digest(workspace: &Path, inputs: &[WorkspacePath]) -> Result<Digest> {
    let mut builder = InputDigestBuilder::new(b"fabrik.exec.script.input.v1\0");
    for input in inputs {
        builder
