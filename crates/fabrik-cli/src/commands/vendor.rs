//! `fabrik vendor` - generate per-crate `fabrik.star` declarations
//! from a workspace's `Cargo.lock` so the granular pipeline can
...
//! - Walks every crates.io package in the resolve and emits a
//!   `cargo_crate(...)` declaration into `vendor/fabrik.star`,
//!   pointing at the registry source path.
...
//! - Vendor sources into the workspace. We point at the existing
//!   `~/.cargo/registry/src/...` cache because copying every byte
//!   would be expensive and adds no portability when the cargo
//!   cache itself is per-machine. A future flag could opt into
//!   copying.
...
pub async fn vendor(workspace: &Path, format: Format) -> Result<ExitCode> {
    ...
    let starfile = vendor_dir.join("fabrik.star");
    tokio::fs::write(&starfile, &report.starfile_body)
        .await
        .with_context(|| format!("writing {}", starfile.display()))?;

    match format {
        Format::Human => {
            let mut err = tokio::io::stderr();
            let summary = format!(
                "fabrik: vendor wrote {path} ({declared} declared, {skipped} skipped)\n  declared: pure-rust libs and proc-macros without build scripts\n  skipped:  crates with build.rs (need cargo_build_script wiring)\n  cargo_binary remains the production path until the third-party graph is feature-complete\n",
                path = starfile.display(),
        ...
        Format::Json | Format::Toon => {
            ...
            let payload = serde_json::json!({
                "starfile": starfile.display().to_string(),
                "declared": report.declared,
                "skipped": report.skipped,
...
fn plan_vendor(metadata: &CargoMetadata) -> Result<VendorReport> {
    ...
    body.push_str(
        "# Generated by `fabrik vendor`. Edit at your peril; re-run the\n\
         # command instead. Sources are referenced from the cargo registry\n\
         # cache, so a `cargo fetch --locked` run must precede a build.\n\
         #\n\
         # Crates with a build.rs (or that pull one through build-deps)\n\
         # are commented out; they need cargo_build_script wiring that is\n\
...
fn write_declared(
    ...
    let _ = writeln!(out, "    proc_macro = {},", py_bool(is_proc_macro));
    out.push_str("    srcs = [],  # add glob([\"src/**/*.rs\"]) once vendored\n");
