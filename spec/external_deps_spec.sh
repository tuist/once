...
Describe 'external dependency consumption'
  ...
  It 'lets a Rust binary import and consume a cargo graph dependency'
    mkdir -p "$WORKSPACE/app/src" "$WORKSPACE/.fabrik/external/cargo/dep_message/src"
...
EOF
    cat > "$WORKSPACE/.fabrik/external/cargo/fabrik.toml" <<'EOF'
[[rust.library]]
name = "dep_message"
srcs = ["dep_message/src/lib.rs"]
EOF
    cat > "$WORKSPACE/.fabrik/external/cargo/dep_message/src/lib.rs" <<'EOF'
...
EOF
  ...
  It 'lets an Elixir binary import and consume a mix graph dependency'
    ...
    mkdir -p "$WORKSPACE/app/lib" "$WORKSPACE/.fabrik/external/mix/external_dep/lib"
...
EOF
    cat > "$WORKSPACE/.fabrik/external/mix/fabrik.toml" <<'EOF'
[[elixir.library]]
name = "external_dep"
srcs = ["external_dep/lib/external_dep.ex"]
EOF
    cat > "$WORKSPACE/.fabrik/external/mix/external_dep/lib/external_dep.ex" <<'EOF'
...
EOF
  ...
  It 'lets a Swift command line app import and consume a SwiftPM graph dependency'
    ...
    mkdir -p "$WORKSPACE/app" "$WORKSPACE/.fabrik/external/swiftpm"
    cat > "$WORKSPACE/fabrik.toml" <<'EOF'
[[deps]]
name = "swiftpm"
ecosystem = "swift"
...
EOF
    cat > "$WORKSPACE/.fabrik/external/swiftpm/fabrik.toml" <<'EOF'
[[apple.swift_library]]
name = "ExternalKit"
...
EOF
    cat > "$WORKSPACE/.fabrik/external/swiftpm/ExternalKit.swift" <<'EOF'
