...
Zig static library target.

## Description

Compiles a Zig static library with `zig build-lib`. The root can be set with
`main`, or omitted when `deps` contains exactly one Zig module dependency.
The output is exposed as a C provider and Apple-linkable archive so native
consumers can link it.
...
| `emit_bin` | bool | no | `true` | Emit the static library. Disable only when another emitted output is enabled |
...
The target emits `zig_static_library`, `c_provider`, `native_linkable`, and
`apple_linkable`.
...
| --- | --- |
| `build` | `static_library`, `asm`, `llvm_ir`, `llvm_bc`, `zig_docs` |
