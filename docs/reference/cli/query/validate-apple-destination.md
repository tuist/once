# `once query validate-apple-destination`
...
```text
once query validate-apple-destination [OPTIONS]
```
...
| --- | --- | --- | --- |
| `--target` | `<TARGET>` |  | Target id the destination will be used with |
| `--destination-kind` | `<DESTINATION_KIND>` |  | Destination kind: `simulator` or `device` |
| `--destination-id` | `<DESTINATION_ID>` |  | Stable destination id returned by `query apple-destinations` |
