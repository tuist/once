---
layout: home

hero:
  name: "Once"
  text: "Run once. Reuse anywhere."
  tagline: Once makes repository automation reusable across developers, coding agents, and machines by giving each action explicit inputs, outputs, environment, and execution policy.
  image:
    src: /logo.png
    alt: Once
  actions:
    - theme: brand
      text: Get started
      link: /guide/getting-started
    - theme: alt
      text: Why Once
      link: /guide/why
    - theme: alt
      text: Remote execution
      link: /guide/infrastructure/remote-execution
    - theme: alt
      text: Reference
      link: /reference/
features:
  - title: Start with a script
    details: Add an input and output contract to automation that already works, then run it through Once.
  - title: Reuse the result
    details: Restore content-addressed outputs locally or through a shared cache when the declared inputs have not changed.
  - title: Run somewhere else
    details: Send only declared inputs to a fresh local or hosted sandbox, retrieve declared outputs, and delete the machine.
  - title: Grow into a graph
    details: Move builds, tests, and linting into typed targets when they need richer dependencies, queries, normalized results, and diagnostics.
---
