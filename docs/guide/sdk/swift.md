...
Download `Once.swift` from the same release and add it to the Swift target
that depends on `Once`. The wrapper imports the C module from the binary
target and gives callers the high-level Swift API.
