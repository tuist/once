...
Every rule's domain logic, including SDK names, triple format, file
extension routing, and toolchain discovery, is written in Starlark on
top of those primitives.

Today the prelude ships Apple rules:

- [`apple_library`](/guide/graph/apple): mixed Swift, Objective-C, C,
  and C++ static library with module emission, bridging headers,
  modulemap generation, `exported_deps`, transitive provider
  propagation, and configurable Xcode and SDK selection.
...
`query` commands are deliberately broad, so agents and humans can both
...
commands; in practice the provider carries `SwiftInfo`-style
transitive views of archives, swiftmodules, linkopts, defines,
frameworks, and modulemaps.

## Action Cache and Execution

Every action, whether it came from a script or a graph rule, goes
through `once_core::run_with_cache`, which keys on a deterministic
action digest composed from:
...
prelude, anticipating a `select()` and platform-transitions story.
