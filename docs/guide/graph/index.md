...
software, or launch a process, so Once only advertises the run tool when
the server is started with an explicit opt-in:

```sh
once mcp --allow-run
```

With that opt-in, agents can call `once_run_target` with the same target
id the CLI accepts. Without it, the MCP surface remains read-oriented and
the run tool is not listed.
