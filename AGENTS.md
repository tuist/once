...
`vendor/fabrik.toml` containing one granular Rust declaration per
crates.io dep. Pure-rust libraries and proc-macros without build
...
copy sources today, so declarations keep empty `srcs` placeholders
until source copying exists.
...
# Generate vendor/fabrik.toml from cargo metadata:
