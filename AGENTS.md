...
- The `__fabrik__/` directory at the workspace root is generated by
  `fabrik deps sync` and contains synthetic external dependency
  packages and dependency graph outputs. It is gitignored.
...
metadata under `__fabrik__/deps/`. Rust sync also emits
`__fabrik__/external/<graph>/fabrik.toml` containing one granular Rust
